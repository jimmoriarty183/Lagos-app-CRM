...
function buildOrderHref(orderId: string, businessSlug?: string, phoneRaw?: string) {
...
export default function OwnerAnalyticsPanel({
  ...
  const getAlertAction = (alert: OwnerDashboardData["alerts"][number]) => {
    if (alert.scope === "manager") {
      ...
      return {
        href,
        linkLabel: managerName ? `Open manager: ${managerLabel(managerName)}` : "Open manager details",
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        <div>
          <div className="text-xs text-[#6B7280]">Owner Analytics / {currentViewMeta.label}</div>
          <div className="mt-1 flex items-center gap-2">
            <h2 className="text-sm font-semibold text-[#111827]">{currentViewMeta.label}</h2>
            <details className="relative">
              ...
              <div className="absolute left-0 top-7 z-20 w-[340px] rounded-xl border border-[#E5E7EB] bg-white p-3 text-xs text-[#344054] shadow-[0_12px_24px_rgba(16,24,40,0.12)]">
                <div className="text-sm font-semibold text-[#111827]">{currentViewMeta.helpTitle}</div>
                <ul className="mt-2 space-y-1">
                  {currentViewMeta.helpPoints.map((point, idx) => (
                    <li key={`${view}-help-${idx}`}>{idx + 1}. {point}</li>
                  ))}
                </ul>
              </div>
            </details>
          ...
          <table className="min-w-full text-left text-xs">
            ...
            <tbody>
              {tableManagers.map((manager) => (
                <tr id={`manager-${manager.manager_id}`} key={manager.manager_id} className="border-t border-[#E5E7EB]">
              ...
              <li key={task.task_id} className="rounded-lg border border-[#E5E7EB] p-2">
                <div className="font-medium text-[#111827]">
                  {buildOrderHref(task.task_id, businessSlug, phoneRaw) ? (
                    <a
                      href={buildOrderHref(task.task_id, businessSlug, phoneRaw) ?? "#"}
                ...
                <div className="mt-1 text-[#6B7280]">
                  Manager:{" "}
                  {buildManagerHref(task.manager_id ?? "") && task.manager_id ? (
                    <a
      ...
      {showReports ? (
      <article className="rounded-xl border border-[#E5E7EB] p-3">
        <div className="mb-2 text-sm font-semibold text-[#111827]">Manager Daily Reports</div>
        <p className="mb-2 text-xs text-[#6B7280]">
                      ...
                      <details>
                        ...
                        <ul className="mt-1 max-w-[260px] space-y-1 rounded bg-[#F9FAFB] p-2 text-[11px] text-[#374151]">
                          {report.completed_today_items.length === 0 ? (
                            ...
                            report.completed_today_items.map((item, idx) => (
                              <li key={`${report.report_id}-done-${idx}`}>
                                {item.href ? (
                                  <a
                                    href={buildOrderHref(item.href, businessSlug, phoneRaw) ?? "#"}
                                    className="text-[#364FC7] hover:underline"
                                  >
                                    {item.label}
                                  </a>
                        ...
                        <ul className="mt-1 max-w-[260px] space-y-1 rounded bg-[#F9FAFB] p-2 text-[11px] text-[#374151]">
                          {report.tomorrow_plan_items.length === 0 ? (
                            ...
                            report.tomorrow_plan_items.map((item, idx) => (
                              <li key={`${report.report_id}-tomorrow-${idx}`}>
                                {item.href ? (
                                  <a
                                    href={buildOrderHref(item.href, businessSlug, phoneRaw) ?? "#"}
                                    className="text-[#364FC7] hover:underline"
      ...
      <article className="space-y-3 rounded-xl border border-[#E5E7EB] p-3">
        <div className="flex items-center justify-between">
          <div>
            <div className="text-sm font-semibold text-[#111827]">Productivity Dashboard</div>
            <div className="text-xs text-[#6B7280]">
              Closed orders and follow-ups from {data.productivity.start_date} to {data.productivity.end_date}
            </div>
          </div>
          {productivityHrefs ? (
            <div className="inline-flex rounded-lg border border-[#E5E7EB] bg-white p-1">
              {([
                ...
                { key: "week", label: "Week", href: productivityHrefs.week },
                { key: "month", label: "Current Month", href: productivityHrefs.month },
              ] as const).map((period) => (
                <a
                  key={period.key}
                  href={period.href}
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
          <article className="rounded-xl border border-[#E5E7EB] p-3">
            <div className="text-xs font-medium text-[#6B7280]">Team Closed Orders</div>
            <div className="mt-1 text-2xl font-semibold text-[#111827]">{data.productivity.team_closed_orders}</div>
          </article>
          <article className="rounded-xl border border-[#E5E7EB] p-3">
            <div className="text-xs font-medium text-[#6B7280]">Team Closed Follow-ups</div>
            <div className="mt-1 text-2xl font-semibold text-[#111827]">{data.productivity.team_closed_followups}</div>
          </article>
          <article className="rounded-xl border border-[#E5E7EB] p-3">
            <div className="text-xs font-medium text-[#6B7280]">Total Closed</div>
            <div className="mt-1 text-2xl font-semibold text-[#111827]">{data.productivity.team_total_closed}</div>
          </article>
        </div>

        <div className="rounded-xl border border-[#E5E7EB] p-3">
          <div className="mb-2 text-sm font-semibold text-[#111827]">By Manager</div>
          <div className="space-y-2">
            {data.productivity.managers.map((manager) => {
              const max = Math.max(1, ...data.productivity.managers.map((x) => x.total_closed));
              const width = (manager.total_closed / max) * 100;
              return (
                <div key={manager.manager_id} className="rounded-lg border border-[#E5E7EB] p-2">
                  <div className="flex items-center justify-between text-xs">
                    <span className="font-medium text-[#111827]">{managerLabel(manager.manager_name)}</span>
                    <span className="text-[#4B5563]">
                      Orders {manager.closed_orders} • Follow-ups {manager.closed_followups} • Total {manager.total_closed}
                    </span>
                  </div>
