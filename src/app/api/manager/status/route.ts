...
async function loadPerson(admin: SupabaseClient, userId: string): Promise<PersonProfile | null> {
  ...
  if (!fullName || !email) {
    const { data: authUserResp } = await admin.auth.admin.getUserById(userId);
    const authUser = authUserResp?.user;

    if (!fullName) {
      const meta = authUser?.user_metadata ?? {};
      const metaFull = clean((meta as Record<string, unknown>).full_name);
      const firstName = clean((meta as Record<string, unknown>).first_name);
      const lastName = clean((meta as Record<string, unknown>).last_name);
      const joined = [firstName, lastName].filter(Boolean).join(" ").trim();
      fullName = metaFull || joined || null;
    }

    if (!email) {
      email = clean(authUser?.email).toLowerCase() || null;
    }
  }
...
export async function GET(req: Request) {
  ...
  const ownerProfile = ownerUserId ? await loadPerson(admin, ownerUserId) : null;

  // manager ACTIVE
  if (managerUserId) {
    const managerProfile = await loadPerson(admin, managerUserId);
    ...
    return NextResponse.json({
      ...
      manager: {
        ...
        full_name: managerProfile?.full_name ?? null,
        phone: null,
        email: managerProfile?.email ?? null,
      },
    });
  }
  ...
  if (accepted?.accepted_by) {
    const managerProfile = await loadPerson(admin, accepted.accepted_by);
    ...
    return NextResponse.json({
      ...
      manager: {
        ...
        full_name: managerProfile?.full_name ?? null,
        phone: null,
        email: managerProfile?.email ?? accepted.email ?? null,
      },
    });
  }
