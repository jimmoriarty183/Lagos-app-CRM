...
export function CampaignDeliveryBadge({ channels }: DeliveryProps) {
  ...
  if (mode === "both") {
    return <Badge className="border-indigo-200 bg-indigo-50 text-indigo-700">Bell + Popup</Badge>;
  }
  if (mode === "popup_only") {
    return <Badge className="border-sky-200 bg-sky-50 text-sky-700">Popup</Badge>;
  }
  return <Badge className="border-slate-200 bg-slate-100 text-slate-700">Bell</Badge>;
}

export function CampaignStatusBadge({ status }: { status: Campaign["status"] }) {
  if (status === "active") return <Badge className="border-emerald-200 bg-emerald-50 text-emerald-700">Sent</Badge>;
  if (status === "archived") return <Badge className="border-slate-200 bg-slate-100 text-slate-700">Archived</Badge>;
  return <Badge className="border-amber-200 bg-amber-50 text-amber-700">Draft</Badge>;
}
