        ...
        <div key={question.id} className="rounded-xl border border-slate-200 bg-white p-4">
          ...
          <div className="mt-3 space-y-2">
            {question.options.map((option) => (
              <div key={option.id} className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
                <div className="flex items-center justify-between gap-3 text-sm">
                  <span className="text-slate-700">{option.label}</span>
                  <span className="font-semibold text-slate-900">{option.responsesCount}</span>
                </div>
              </div>
            ))}
          </div>
